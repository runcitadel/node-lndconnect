...
test('encodeMacaroon (data)', async (t) => {
  ...
  const macaroonFile = await fs.readFile(macaroonPath);
...
test('parse', (t) => {
  ...
  t.equal(connectionString.host, `${HOSTNAME}:${PORT}`, 'generated expected host string');
  t.equal(connectionString.cert, ENCODED_CERT, 'generated expected cert string');
  t.equal(connectionString.macaroon, ENCODED_MACAROON, 'generated expected macaroon string');
});
...
test('decode (Invalid lndconnect url)', (t) => {
  t.plan(1);
  t.throws(() => decode('not' + CONNECTION_STRING), /Invalid lndconnect url/, 'throws an "Invalid lndconnect url" error');
});
